...
确认文件具有 `name: jolink-java` 的 frontmatter 和正文。如果客户端中该 Skill
被禁用，启用它。不安装另一份中文 Skill，也不额外写全局规则；这份 Skill 就是使用指引。

## 5. 重连并验证

在客户端 MCP 设置中重载 joLink，然后：

1. 确认服务连接成功，客户端能列出工具。查看实际工具 schema，不假定旧版本具有新版接口。
2. 调用一次可用的状态工具：`java_status(action="status")`；服务提供旧接口时用
   `java_runtime(action="status")`。不为了检查安装而启动或停止业务应用。
3. 确认客户端的 Skill 列表中能看到 `jolink-java`，或能从客户端加载它。

当前对话无法刷新工具/Skill 列表时，先完成文件配置，再告诉用户在这个客户端重连或
新建会话，不要直接杀掉 Agent 进程。原生 `mcp list` 显示已保存的配置，只能证明
注册完成，不能单独证明服务器连接成功。

最后简短报告：配置了哪个客户端、Skill 装在哪里、MCP 和 Skill 加载是否已验证，
以及还需要用户完成的那一步。只写好了配置时，不要说“已经连接成功”。
