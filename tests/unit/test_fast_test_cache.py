...
def test_generator_inputs_use_stat_and_track_add_edit_delete(tmp_path, monkeypatch):
    from jolink_runtime.launch.configuration_inputs import preparation_stamps

    root = tmp_path / "syntax"
    root.mkdir()
    grammar = root / "Language.g4"
    grammar.write_text("original")
    monkeypatch.setattr(
        Path,
        "read_bytes",
        lambda _self: (_ for _ in ()).throw(
            AssertionError("must not hash generator input")
        ),
    )
    before = preparation_stamps((root,))
    grammar.write_text("changed grammar")
    assert preparation_stamps((root,)) != before
    before = preparation_stamps((root,))
    extra = root / "Added.g4"
...
def test_source_registration_does_not_reprobe_for_java_changes(tmp_path):
    from jolink_runtime.launch.configuration_inputs import preparation_stamps

    root = tmp_path / "checks"
    root.mkdir()
    before = preparation_stamps((), (root,))
    source = root / "Test.java"
    source.write_text("class Test {}")
    assert preparation_stamps((), (root,)) == before
    source.write_text("class Test { int changed; }")
    assert preparation_stamps((), (root,)) == before
    source.unlink()
    root.rmdir()
    assert preparation_stamps((), (root,)) != before
...
def test_preparation_metadata_does_not_change_compiler_identity():
    from jolink_runtime.launch.jdt_modules import compilation_modules

    before = ({"source_roots": ["src"], "classpath": ["lib.jar"]},)
    after = (
        {
            **before[0],
            "preparation_inputs": ["grammar"],
            "preparation_roots": [],
            "preparation_executions": ["generator:generate"],
        },
    )
    assert compilation_modules(before) == compilation_modules(after)
    assert "preparation_inputs" in after[0]
    after[0]["source_roots"] = ["src", "generated"]
    assert compilation_modules(before) != compilation_modules(after)
...
def test_gradle_cache_uses_exported_subproject_build_files(tmp_path):
    ...
    script.write_text(
        'plugins { `java-library` }; dependencies { implementation("g:a:1") }'
    )
    assert _inputs(tmp_path, "gradle", (script,)) != before
    assert _configuration_stamps(startup) != startup
